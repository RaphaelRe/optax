    ...
    def _orthogonalize(x):
        ...
        if ns_coeffs_.ndim == 1:
            x = jax.lax.fori_loop(
                0,
                ns_steps,
                lambda _, x: _newton_schulz_iterator(x, ns_coeffs_),
                x,
                unroll=True,
            )  # Unroll to ensure efficient composition with jax.vmap.
        else:
            x, _ = jax.lax.scan(
                lambda x, abc: (_newton_schulz_iterator(x, abc), None),
                x,
                ns_coeffs_,
            )
        if transposed:
...
def muon(
    ...
    *,
    ...
    Args:
      ...
      learning_rate_adam: Learning rate for parts with AdamW optimizer parts.
         A `None` value implies the same learning rate for Muon and AdamW.
